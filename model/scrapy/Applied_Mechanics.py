...
class AppliedMechanicsCrawler(QObject):
    journal_name = "Applied_Mechanics"
    ...
    def get(self, str, csv_write, HEADERS, out, proxy_info):
        row = []
    ...
    def if_have_next(self, Volumeurl, HEADERS, proxies2):
    ...
    def obtain_info(self):
        ...
        out = open(self.abspath + "/../data/journal_total/Applied_Mechanics.csv", 'w', newline='', encoding='UTF-8')
        ...
        for i in range(89, 88, -1):
            for j in range(1, 12):
                ...
                while next is not None:
                    next = self.if_have_next(Volumeurl, HEADERS, proxies2)
                    if next is None:
                        continue
                    else:
        ...
        for i in range(0, len(Article_sort)):
            ...
            if num % 5 == 0:
                ...
                pass
            self.get(url_art, csv_write, HEADERS, out, proxies3)
            time.sleep(0.5)
        print('over')
        out.close()
        # 获取信息结束，发送结束信号
